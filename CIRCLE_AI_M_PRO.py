...
with st.sidebar:
    ...
    mode = st.selectbox("", 
        ["📈 Saham (Yahoo)", "💱 Forex (AlphaV)", "🪙 Crypto (CoinGecko)"],
    ...
    elif mode == "💱 Forex (AlphaV)":
        col1, col2 = st.columns(2)
        with col1:
            base = st.text_input("Dari", value="USD").upper()
        with col2:
            quote = st.text_input("Ke", value="IDR").upper()
        ticker = f"{base}/{quote}"
    else:  # Crypto
        crypto_options = {
            "Bitcoin (BTC)": "bitcoin",
...
with st.spinner('⏳ Memuat data pasar...'):
    ...
    if mode == "📈 Saham (Yahoo)":
        ...
        if not data_yf.empty:
            ...
            err = "Ticker tidak ditemukan"
            
    elif mode == "💱 Forex (AlphaV)":
        # Note: Alpha Vantage free tier = daily only. For H1/H4 need premium or different API
        if "H1" in timeframe or "H4" in timeframe:
            st.warning("⚠️ Forex intraday butuh API premium. Beralih ke daily.")
        ...
        try:
            ...
            if "Time Series FX (Daily)" in data:
                ...
                err = "API Limit reached atau key invalid"
        except:
            err = "Koneksi gagal"
